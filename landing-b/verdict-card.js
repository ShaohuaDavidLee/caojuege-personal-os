/* 太史判词卡 · Canvas 2D 渲染器（灰底·正典）
 * 单一来源，被 create.html / receive.html / index.html 复用。
 * 3:4 (1080×1440)。规避 html2canvas 的中文叠字问题。 */
(function (g) {
  'use strict';
  ...
  function hash(seed){ var h = 2166136261; var str = String(seed); for (var i = 0; i < str.length; i++){ h ^= str.charCodeAt(i); h = Math.imul(h, 16777619); } return Math.abs(h); }
  ...
  function buildData(name, b1, b2, off){
    name = clean(name) || '某某';
    return {
      name: name,
      line: buildLine(name, b1, b2, off),
  ...
  function drawSeal(ctx, x, y, w, h){
    ...
    ctx.fillText(ch[0], x + w * 0.3, y + h * 0.3);
    ctx.fillText(ch[1], x + w * 0.7, y + h * 0.3);
    ctx.fillText(ch[2], x + w * 0.3, y + h * 0.7);
    ctx.fillText(ch[3], x + w * 0.7, y + h * 0.7);
    ctx.restore();
  }
  ...
  function drawQR(ctx, x, y, s){
    ...
    var pad = Math.round(s * 0.08), n = 21, cell = (s - pad * 2) / n, ox = x + pad, oy = y + pad;
    ctx.fillStyle = '#1a1a1a';
    var h = hash('s.caojuege.com');
  ...
  function draw(canvas, d){
    ...
    // watermark 判詞（繁体，竖排两字）
    ctx.save();
    ctx.fillStyle = 'rgba(0,0,0,.06)'; ctx.font = '900 440px ' + SERIF; setLS(ctx, 0);
    ...
    // top bar
    ctx.save();
    ctx.strokeStyle = 'rgba(71,73,75,.45)'; ctx.lineWidth = 1.5; ctx.strokeRect(P, 76, 184, 54);
    ctx.fillStyle = '#47494b'; ctx.font = '600 28px ' + SERIF; setLS(ctx, 0);
    ctx.textAlign = 'left'; ctx.textBaseline = 'middle'; ctx.fillText('毒判 · ' + d.vol, P + 18, 76 + 28);
    ctx.fillStyle = '#5c5e60'; ctx.font = '500 19px ' + MONO; setLS(ctx, 4);
    ctx.textAlign = 'right'; ctx.fillText('PERSONAL ANNAL · 判', W - P, 76 + 27);
    ctx.restore();

    // seal (taller chop, upper right)
    drawSeal(ctx, W - P - 150, 180, 150, 172);

    // verdict (centered in zone)
    ctx.save();
    ctx.fillStyle = '#1a1a1a'; ctx.font = '700 72px ' + SERIF; setLS(ctx, 1);
    ctx.textAlign = 'left'; ctx.textBaseline = 'top';
    var lineH = 108, maxLines = 6;
    var lines = wrap(ctx, '「' + d.line + '」', W - 2 * P, maxLines);
    var zT = 440, zB = 1012;
    var y = Math.max(zT, (zT + zB) / 2 - (lines.length * lineH) / 2);
    for (var i = 0; i < lines.length; i++){ ctx.fillText(lines[i], P, y); y += lineH; }
    ctx.restore();

    // divider + evidence
    ctx.save();
    ctx.fillStyle = 'rgba(0,0,0,.16)'; ctx.fillRect(P, 1078, W - 2 * P, 2);
    ctx.fillStyle = '#5c5e60'; ctx.font = '400 25px ' + SERIF; setLS(ctx, 0);
    ctx.textAlign = 'left'; ctx.textBaseline = 'top';
    var ey = 1108, evl = wrap(ctx, d.ev, W - 2 * P, 2);
    for (var j = 0; j < evl.length; j++){ ctx.fillText(evl[j], P, ey); ey += 38; }
    ctx.restore();

    // footer
    drawQR(ctx, W - P - 158, H - P - 158, 158);
    ctx.save();
    ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
    ctx.fillStyle = '#5c5e60'; ctx.font = '400 22px ' + SERIF; setLS(ctx, 0);
    ctx.fillText('司马迁.skill · 太史判词', P, H - P - 104);
    ctx.fillStyle = '#1a1a1a'; ctx.font = '600 38px ' + MONO; setLS(ctx, 1);
    ctx.fillText('s.caojuege.com', P, H - P - 52);
    ctx.fillStyle = '#5c5e60'; ctx.font = '500 17px ' + MONO; setLS(ctx, 2);
    ctx.fillText('扫码也给朋友定一句', P, H - P - 16);
    ctx.restore();
  }
  ...
  function ensureFonts(){
    if (document.fonts && document.fonts.load){
      return Promise.all([
        ...
        document.fonts.load('500 25px "Noto Serif SC"')
      ]).catch(function(){});
    }
  ...
  g.VerdictCard = { W: W, H: H, buildLine: buildLine, buildData: buildData, draw: draw, ensureFonts: ensureFonts, download: download, clean: clean, vol: vol };
})(window);
